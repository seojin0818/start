...
/*
	...
	즉,
		...
		2번 스레드는 이곳에서 데이터를 가지고 갈 예정
		
	규칙
		5개의 데이터가 입력되면 더 이상 입력하면 안 됨
...
public class Synch02 {
	...
	synchronized void makeData(int num) {
		if(list.size() == 5) {
			// 이 경우 데이터가 5개가 채워져 있으므로 작업을 하면 안됨
			try {
		...
		System.out.println("<> 채운 후 데이터의 개수는 " + list.size() + " 개가 있습니다.");
		// 이렇게 숫자를 채우면 숫자를 사용할 수 있는 상태가 됨
		notify();
		// ==> 가장 먼저 Block 상태로 들어간 스레드가 일을 하게 됨
	...
	synchronized void useData() {
		...
		int no = list.get(0);
		// 꺼낸 데이터는 리스트에서 제거함
		list.remove(0); // 꺼낸 데이터를 제거했으므로 입력할 수 있는 공간이 하나가 더 생겨남
		System.out.println("\t\t" + no + " 를 꺼냈습니다.");
		System.out.println("\t\t남아있는 데이터의 개수는 " + list.size() + " 개입니다.");
		// 이제 데이터를 추가할 수 있는 공간이 추가되었으니
		// 다시 데이터를 채우는 작업을 깨우러 감
		notify();
