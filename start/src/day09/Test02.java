...
//		return; // 이 명령을 기술하지 않아도 실행내용을 모두 실행했으므로 이 함수가 자동 종료될 것
		// 따라서 생략해도 똑같은 효과가 됨
	}
	...
	public void toPrint(int no1, int no2, int no3) {
		...
		//		이미 만들어놨으므로 따로 안만들어도 된다는 의미
		// 함수의 오버로딩(함수의 중첩) toPrint 똑같이 쓰면 안되는게 원칙이지만 허용
		...
		/*
		 	...
		 	함수의 오버로딩
		 		...
		 		원칙 ] ***** 암기
		 			...
		 			0. 함수에 오버로딩 하려는 함수가 있어야함
		 			1. 함수이름이 같아야함
		 			2. 매개변수리스트가 달라야함
		 				(매개변수리스트의 타입, 순서, 개수 중 하나 이상이 달라야함)
		 			3. 반환값타입과는 무관함
